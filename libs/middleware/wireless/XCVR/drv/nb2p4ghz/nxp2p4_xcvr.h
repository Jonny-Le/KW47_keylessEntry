...
#ifndef IQMC_TRIM_DOWNLOAD_SUPPORT
#if defined(NXP_RADIO_GEN) && (NXP_RADIO_GEN==470)
#define IQMC_TRIM_DOWNLOAD_SUPPORT (1)
#else 
#define IQMC_TRIM_DOWNLOAD_SUPPORT (0)
#endif      
#endif /* ifndef IQMC_TRIM_DOWNLOAD_SUPPORT */

/* Error check compile flags */
#if !defined(RF_OSC_26MHZ)
...
#define TSM_TX_HI(x)   (((uint32_t)(((uint32_t)(x)) << 0)) & 0xFFU)
#define TSM_TX_LO(x)   (((uint32_t)(((uint32_t)(x)) << 8)) & 0xFF00U)
#define TSM_RX_HI(x)   (((uint32_t)(((uint32_t)(x)) << 16)) & 0xFF0000U)
#define TSM_RX_LO(x)   (((uint32_t)(((uint32_t)(x)) << 24)) & 0xFF000000U)
...
#define XTAL_TEMP_COMP_LUT_SIZE \
    (17) /*!< Default LUT size for temp compensation of XTAL trim (signed, but must be positive) */
#endif
...
/*!
 * @brief Fetches  the current active data rate for coded Bluetooth LE.
 *
 * This function reads the data rate status for coded operation in Bluetooth LE..
 *
 * @return Returns 1 if the coded data rate is 500Kbps (S2); Returns 0 if the coded data rate is 125Kbps (S8).
 *
 * @warn This routine does not check that coding is active and will return invalid data for non-coded cases. The results from this routine are only
 * valid after the Link Layer hardware has started operation.
 */
uint8_t XCVR_GetCodedDataRate(void);
 ...
 * @return The status of the trim process.
 */
xcvrStatus_t XCVR_SetXtalTempComp(const xcvr_temp_comp_lut_t * temp_comp_lut, int16_t temp_deg_c);
...
/*!
 * @brief Function to set RCCAL to a manual value.
 *
 * This function sets full manual control of RCCAL.
 *
 * @param[in] man_rccal_val  The desired manual RCCAL value to use (< 0x1F).
 *
 * @return The status of the manual RCCAL enable.
 */
xcvrStatus_t XCVR_SetManRCcal(uint8_t man_rccal_val);

/*!
 * @brief Function to release RCCAL manual setting.
 *
 * This function clears the enable for manual RCCAL application and allows RCCAL automatic processing to operate normally.
 *
 */
 void XCVR_ReleaseManRCcal(void);
 ...
 */
xcvrStatus_t XCVR_FastPeriphDescrip_Load(const uint32_t *descriptor_list,
...
/*!
 * @brief Function to snapshot or restore XCVR registers to a storage structure in PKT RAM using fast IPS
 * DMA.
 *
 * This function captures or restores the state of the XCVR registers with a fast DMA block storing to one of the
 * PKT RAM banks. It is used to quickly backup and restore XCVR registers without the slow access time penalty for normal
 ...
 *
 * @return The status of the XCVR fast backup process.
 *
 * @pre The caller must have called the XCVR_ValidateFastPeriphDescrip() routine to validate the descriptor structure is
 * correct and PKT RAM limits are not exceeded. The function XCVR_FastPeriphDescrip_WordCount() can be used to get the
 * word count that will be used in the PKT RAM for storage of the data.
 *
 */
xcvrStatus_t XCVR_FastBackupRestore(const uint32_t *desc_ptr,
                                    uint8_t num_entries,
                                    PKT_RAM_BANK_SEL_T pkt_ram_bank,
                                    uint16_t pkt_ram_index_offset,
                                    bool restore);

#if defined(SAVE_RESTORE_15P4)

#define XCVR_15P4_BACKUP_DESC_SIZE      11U

extern uint32_t xcvr_15p4_backup_descriptors[XCVR_15P4_BACKUP_DESC_SIZE];

void XCVR_Create15p4BackupDescriptors();

#endif

#if defined(SAVE_RESTORE_BLE)

#define XCVR_BLE_BACKUP_DESC_SIZE       11U

extern uint32_t xcvr_ble_backup_descriptors[XCVR_BLE_BACKUP_DESC_SIZE];

void XCVR_CreateBleBackupDescriptors();

#endif

/*!
 * @brief Function to backup the XCVR LIF and RBME registers to the TX_PACKET_RAM using the fast IPS DMA.
 *
 * The function XCVR_FastPeriph_WaitComplete() can be used to ensure the DMA transfer is complete.
 *
 * @return The status of the XCVR fast backup process.
 */
xcvrStatus_t XCVR_LIF_RBME_Backup(void);

/*!
 * @brief Function to restore the XCVR LIF and RBME registers from the TX_PACKET_RAM using the fast IPS DMA.
 *
 * The function XCVR_FastPeriph_WaitComplete() can be used to ensure the DMA transfer is complete.
 *
 * @return The status of the XCVR fast backup process.
 */
xcvrStatus_t XCVR_LIF_RBME_Restore(void);
