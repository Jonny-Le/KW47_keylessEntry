...
void BluetoothLEHost_AppInit(void)
{
    ...
    /* Initialize RSSI proximity integration */
...
void APP_UserInterfaceEventHandler(void *pData)
{
    ...
    switch(pEventData->appEvent)
    {
        ...
        {
            shell_write("\r\nDigital Key Car Anchor by Hoc Le - Build: " __DATE__ " " __TIME__ "\r\n");
            shell_cmd_finished();
        }
        break;
...
void App_HandleShellCmds(void *pData)
{
    ...
    switch(pEventData->appEvent)
    {
        ...
        case mAppEvt_Shell_RssiStatus_Command_c:
        {
            int16_t emaQ4;
            uint16_t stdQ4, pctQ15, sampleCnt;
            int32_t emdBx10, stdDBx10, pctVal;

            shell_write("\r\n========== RSSI Status ==========\r\n");
            
            emaQ4 = RssiIntegration_GetEmaQ4();
            stdQ4 = RssiIntegration_GetStdQ4();
            pctQ15 = RssiIntegration_GetPctAboveEnterQ15();
            sampleCnt = RssiIntegration_GetSampleCount();

            /* Format EMA: Q4 = rssi * 16, so dBm = Q4 / 16 */
            shell_write("EMA RSSI: ");
            if (emaQ4 < 0)
            {
                shell_write("-");
                emdBx10 = (-(int32_t)emaQ4 * 10) / 16;
            }
            else
            {
                emdBx10 = ((int32_t)emaQ4 * 10) / 16;
            }
            shell_writeDec((uint32_t)(emdBx10 / 10));
            shell_write(".");
            shell_writeDec((uint32_t)(emdBx10 % 10));
            shell_write(" dBm\r\n");
            ...
            /* Standard Deviation */
            shell_write("Std Dev:  ");
            stdDBx10 = ((int32_t)stdQ4 * 10) / 16;
            shell_writeDec((uint32_t)(stdDBx10 / 10));
            shell_write(".");
            shell_writeDec((uint32_t)(stdDBx10 % 10));
            shell_write(" dB\r\n");

            /* Percentage above enter threshold */
            shell_write("% Above:  ");
            pctVal = ((int32_t)pctQ15 * 100) / 32767;
            shell_writeDec((uint32_t)pctVal);
            shell_write("%\r\n");

            /* Sample count */
            shell_write("Samples:  ");
            shell_writeDec((uint32_t)sampleCnt);
            shell_write("\r\n");

            /* State */
            shell_write("State:    ");
            shell_write(RssiIntegration_GetStateName());
            shell_write("\r\n");
        ...
        case mAppEvt_Shell_RssiHistory_Command_c:
        {
            shell_write("\r\n======== RSSI History ==========\r\n");
            shell_write("RSSI Proximity Monitoring:\r\n");
            ...
            for (uint8_t i = 0U; i < (uint8_t)gAppMaxConnections_c; i++)
            {
                if (maPeerInformation[i].deviceId != gInvalidDeviceId_c)
                {
                    ...
                    shell_writeDec((uint32_t)maPeerInformation[i].deviceId);
            ...
            if (deviceCount == 0U)
            {
                shell_write("  No connected devices\r\n");
            }
            ...
            shell_write("\nThresholds:\r\n");
            shell_write("  Enter (Near):  -50 dBm\r\n");
            shell_write("  Exit  (Far):   -60 dBm\r\n");
            shell_write("  Hysteresis:    10 dB\r\n");

            shell_write("\nStability Requirements:\r\n");
            shell_write("  >= 40% samples above threshold\r\n");
            shell_write("  Std Dev <= 8 dB\r\n");
            shell_write("  Stable for 2000ms\r\n");
