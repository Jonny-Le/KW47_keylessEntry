...
static shell_status_t ShellRssiStatus_Command(shell_handle_t shellHandle, int32_t argc, char * argv[]);
static shell_status_t ShellRssiHistory_Command(shell_handle_t shellHandle, int32_t argc, char * argv[]);
static shell_status_t ShellRssiTest_Command(shell_handle_t shellHandle, int32_t argc, char * argv[]);
...
static shell_command_t mRssiStatusCmd =
{
    .pcCommand = "rssi",
    .pcHelpString = "\r\n\"rssi\": Display current RSSI status information.\r\n",
    .cExpectedNumberOfParameters = 0,
    .pFuncCallBack = ShellRssiStatus_Command,
};

static shell_command_t mRssiHistoryCmd =
{
    .pcCommand = "rssihistory",
    .pcHelpString = "\r\n\"rssihistory\": Display RSSI history for connected devices.\r\n",
    .cExpectedNumberOfParameters = 0,
    .pFuncCallBack = ShellRssiHistory_Command,
};

static shell_command_t mRssiTestCmd =
{
    .pcCommand = "rssitest",
    .pcHelpString = "\r\n\"rssitest [rssi_value]\": Test RSSI with simulated value (e.g., rssitest -55).\r\n",
    .cExpectedNumberOfParameters = SHELL_IGNORE_PARAMETER_COUNT,
    .pFuncCallBack = ShellRssiTest_Command,
};
...
void AppShellInit(char* prompt)
{
    ...
    assert(kStatus_SHELL_Success == status);
    status = SHELL_RegisterCommand((shell_handle_t)g_shellHandle, &mRssiStatusCmd);
    assert(kStatus_SHELL_Success == status);
    status = SHELL_RegisterCommand((shell_handle_t)g_shellHandle, &mRssiHistoryCmd);
    assert(kStatus_SHELL_Success == status);
    status = SHELL_RegisterCommand((shell_handle_t)g_shellHandle, &mRssiTestCmd);
    assert(kStatus_SHELL_Success == status);
#endif /* defined(gAppUseShellInApplication_d) && (gAppUseShellInApplication_d == 1) */
}
...
/*! *********************************************************************************
* \brief        Display current RSSI status information.
*
* \param[in]    argc           Number of arguments
* \param[in]    argv           Pointer to arguments
*
...
static shell_status_t ShellRssiStatus_Command(shell_handle_t shellHandle, int32_t argc, char * argv[])
{
    if (mpfShellEventHandler != NULL)
    {
        ...
        if(pEventData != NULL)
        {
            pEventData->appEvent = mAppEvt_Shell_RssiStatus_Command_c;
            if (gBleSuccess_c != App_PostCallbackMessage(mpfShellEventHandler, pEventData))
            {
                (void)MEM_BufferFree(pEventData);
            }
...
/*! *********************************************************************************
* \brief        Display RSSI history for connected devices.
*
* \param[in]    argc           Number of arguments
* \param[in]    argv           Pointer to arguments
*
...
static shell_status_t ShellRssiHistory_Command(shell_handle_t shellHandle, int32_t argc, char * argv[])
{
    if (mpfShellEventHandler != NULL)
    {
        ...
        if(pEventData != NULL)
        {
            pEventData->appEvent = mAppEvt_Shell_RssiHistory_Command_c;
            if (gBleSuccess_c != App_PostCallbackMessage(mpfShellEventHandler, pEventData))
            {
                (void)MEM_BufferFree(pEventData);
            }
...
/*! *********************************************************************************
* \brief        Test RSSI with simulated value.
*
* \param[in]    argc           Number of arguments
* \param[in]    argv           Pointer to arguments
*
...
static shell_status_t ShellRssiTest_Command(shell_handle_t shellHandle, int32_t argc, char * argv[])
{
    ...
    if (argc >= 2)
    {
        rssiValue = BleApp_atoi(argv[1]);
        
        if (rssiValue > 0 || rssiValue < -100)
        {
            shell_write("\r\nInvalid RSSI value. Use range -100 to -20 dBm.\r\n");
            return kStatus_SHELL_Success;
        }

        /* Push test RSSI value to ProxRssi */
        RssiIntegration_UpdateRssi(0, (int8_t)rssiValue);
        shell_write("\r\nRSSI test value pushed: ");
        shell_write(rssiValue < 0 ? "-" : "");
        shell_writeDec((uint32_t)(rssiValue < 0 ? -rssiValue : rssiValue));
        shell_write(" dBm\r\n");
    }
    else
    {
        shell_write("\r\nUsage: rssitest <rssi_value>\r\n");
        shell_write("Example: rssitest -55\r\n");
    }
